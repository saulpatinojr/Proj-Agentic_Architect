...
describe('VS Code startup fingerprinting', () => {
  ...
  it('changes the fingerprint when workspace root changes even if config contents match', () => {
    const first = mkdtempSync(join(tmpdir(), 'cc-vscode-root-a-'));
    const second = mkdtempSync(join(tmpdir(), 'cc-vscode-root-b-'));
    try {
      ...
      expect(workspaceFingerprint(first)).not.toBe(workspaceFingerprint(second));
    } finally {
