...
export interface TaskEnvelope { id: string; objective: string; repository?: string; acceptanceCriteria: string[]; risk: RiskClass; constraints: string[]; specializations?: string[]; createdAt: string }
export interface AgentAssignment { id: string; taskId: string; agentId: string; role: string; stance: Stance; provider: string; harness: string; surface?: string; billingChannel: BillingChannel; authority: string[]; dependsOn: string[] }
...
export interface AgentResult { taskId: string; assignmentId: string; agentId: string; role: string; stance: Stance; provider: string; harness: string; surface?: string; billingChannel: BillingChannel; status: 'completed' | 'awaiting_external' | 'blocked' | 'failed' | 'cancelled'; changes: string[]; tests: GateResult[]; evidence: Evidence[]; findings: Finding[]; risks: string[]; blockers: string[]; recommendation: 'continue' | 'review' | 'changes_required' | 'ready' | 'human_required' }
